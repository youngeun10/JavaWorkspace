...
public class _05_Stream {
    public static void main(String[] args) {
        // 스트림 : 데이터 필터링이나 가공할 때 유용 (데이터 소스를 추상화 하여 데이터를 다루는데 자주 사용되는 메서드들을 정의해놓음.
...
//        langList.add("java");
        ...
        // 스트림 사용
        // 스트림에는 중간 연산(Intermediate Operation), 최종 연산(Terminal Operation)이란게 존재
        // 중간 연산(Intermediate Operation) : 여러 번 연산 가능     ex) filter, map, sorted, distinct, skip, ....
        // 최종 연산(Terminal Operation) : 마지막에 한 번 연산    ex) count, min, max, sum, forEach, anyMatch, allMatch, ...
        ...
        // 4글자 이하인 언어 중에서 c 라는 글자를 포함하는 언어
        langList.stream()
                .filter(x-> x.length() <= 4)
        ...
        // 4글자 이하인 언어 중에서 c라는 글자를 포함하는 언어가 하나라도 있는지 여부
        ...
        langList.stream()
                .filter(x -> x.contains("c"))
                .map(String::toUpperCase)       // String 클래스 안에있는 toUpperCase메소드를 사용하겠다~
                .forEach(System.out::println);
        System.out.println("----------------------------------------");
